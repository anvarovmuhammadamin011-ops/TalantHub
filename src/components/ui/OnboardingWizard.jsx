    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 pt-6">
          ...
          <button onClick={skip} className="p-1 text-ink-3 hover:text-ink" title="O'tkazib yuborish">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6">
          {step === 0 && (
            <div className="text-center py-4">
              ...
              <h2 className="text-xl font-semibold text-ink mb-2 tracking-tight">Xush kelibsiz, {user.name.split(" ")[0]}!</h2>
              <p className="text-ink-3 text-sm mb-8 leading-relaxed">
                Keling, profilingizni bir necha qadamda to'ldiramiz. Bu vakansiyalar bilan moslik foizini
                aniqroq hisoblashga va sizga eng mos ishlarni topishga yordam beradi.
              </p>
          ...
          {step === 1 && (
            <div>
              <h2 className="text-lg font-semibold text-ink mb-1.5 tracking-tight">Ko'nikmalaringiz qanday?</h2>
              <p className="text-ink-3 text-sm mb-5">Vakansiyalar bilan moslik foizi shu ko'nikmalar asosida hisoblanadi</p>
              ...
              {suggestions.length > 0 && (
                <div className="mb-2">
                  <div className="text-xs text-ink-3 mb-2">Tavsiya etiladi:</div>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-lg font-semibold text-ink mb-1.5 tracking-tight">O'zingiz haqingizda</h2>
              <p className="text-ink-3 text-sm mb-5">Ish beruvchilar buni profilingizda ko'radi</p>

              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-ink-3 uppercase tracking-wide mb-1.5">Qisqacha bio</label>
                  <textarea
                    value={bio}
                    onChange={(e) => setBio(e.target.value)}
                    rows={3}
                    placeholder="Masalan: 3 yillik tajribaga ega frontend dasturchiman..."
          ...
          {step === 3 && (
            <div>
              <h2 className="text-lg font-semibold text-ink mb-1.5 tracking-tight">Kutilayotgan maosh</h2>
              <p className="text-ink-3 text-sm mb-5">Ixtiyoriy — keyinroq ham to'ldirishingiz mumkin</p>
              <div>
                <label className="block text-xs font-medium text-ink-3 uppercase tracking-wide mb-1.5">Kutilayotgan oylik maosh</label>
              ...
              <div className="mt-6 p-4 bg-accent-soft rounded-xl flex items-start gap-3">
                ...
                <p className="text-sm text-ink-2 leading-relaxed">
                  Ajoyib! Profilingiz {[skills.length > 0, !!bio, !!city, !!salary].filter(Boolean).length + 1}/5 qismga to'ldirildi.
                  Yakunlab, sizga mos vakansiyalarni ko'ring.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 mt-8">
            ...
            {step === totalSteps - 1 && (
              <button onClick={finish} disabled={saving} className="flex-1 bg-ink text-white py-2.5 rounded-lg text-sm font-medium hover:bg-ink/90 transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                {saving ? "Saqlanmoqda..." : "Yakunlash"} <CheckCircle className="w-4 h-4" />
              </button>
            )}
          </div>
        </div>
