...
import { Send, Sparkles, Bot, User, ArrowRight, MapPin, Star, Briefcase } from "lucide-react";
...
const suggestions = [
  ...
  "Toshkentda ingliz tili o'qituvchisi kerak",
  "Senior Python developer, masofaviy ishlash",
];
...
export default function AiChat() {
  ...
  useEffect(() => {
    ...
    setMessages([{ id: 0, role: "ai", text: greeting, specialists: [] }]);
  }, []);
          ...
          <div key={msg.id}>
            {msg.role === "user" ? (
              <div className="flex justify-end">
                <div className="bg-ink text-white px-4 py-2.5 rounded-2xl rounded-br-md max-w-[80%] text-sm">
                  {msg.text}
                </div>
              </div>
            ) : (
                  ...
                  {msg.specialists && msg.specialists.length > 0 && (
                    <div className="mt-4 space-y-3">
                      {msg.specialists.map((s) => (
                        <div
                          key={s.id}
                          className="bg-white rounded-xl border border-border p-4 hover:border-ink/20 hover:shadow-sm transition-all"
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3 mt-1.5 text-xs text-ink-3">
                                <span className="flex items-center gap-1">
                                  <Star className="w-3 h-3 text-ink fill-ink" /> {s.rating}
                                </span>
                                <span className="flex items-center gap-1">
                                  <Briefcase className="w-3 h-3" /> {s.experience}
                                </span>
                                <span className="flex items-center gap-1">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {suggestions.map((s) => (
            <button
              ...
              className="px-3 py-1.5 bg-surface border border-border rounded-full text-xs text-ink-2 hover:border-ink/30 hover:text-ink transition-colors"
            >
              {s}
            </button>
          ))}
      ...
      <div className="border-t border-border pt-4">
        <div className="flex items-center gap-2">
          <input
            ref={inputRef}
            type="text"
            placeholder="Qanday mutaxassis kerak? Masalan: React dasturchi, 5 yil tajriba..."
            value={input}
