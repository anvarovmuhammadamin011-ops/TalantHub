...
export default function AdminDisputes() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <h1 className="text-2xl font-semibold text-ink tracking-tight mb-6">Nizolar</h1>
      ...
      {disputes.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-ink-3">Nizolar yo'q</p>
        </div>
      ) : (
            ...
            <div key={d.id} className="bg-white rounded-xl border border-border shadow-sm p-5">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="font-medium text-ink text-sm">{d.order_title || `Zakaz #${d.order_id}`}</div>
                  ...
                  <div className="text-xs text-ink-3 mt-1">Ochgan: {d.opened_by_name} · {d.order_price}</div>
                  {!!d.reason && <p className="text-sm text-ink-2 mt-2">{d.reason}</p>}
                  {!!d.resolution && (
                    <div className="mt-3 pt-3 border-t border-border-soft">
                      <div className="text-[10px] font-medium text-ink-3 uppercase tracking-wide mb-1">Yechim</div>
                      <p className="text-sm text-ink-2">{d.resolution}</p>
                      {!!d.resolved_by_name && <p className="text-xs text-ink-3 mt-1">— {d.resolved_by_name}</p>}
                    </div>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 mt-4 flex-wrap">
                <button onClick={() => openResolve(d)}
                  ...
                  <CheckCircle2 className="w-3.5 h-3.5" /> {d.resolution ? "Yechimni tahrirlash" : "Hal qilish"}
                </button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setResolving(null)}>
          <div className="bg-white rounded-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-1">
              <h3 className="font-semibold text-ink text-base">{resolving.order_title || `Zakaz #${resolving.order_id}`}</h3>
              <button onClick={() => setResolving(null)} className="text-ink-3 hover:text-ink"><X className="w-5 h-5" /></button>
            </div>
            <p className="text-xs text-ink-3 mb-4">Sabab: {resolving.reason}</p>
            <textarea value={resolutionText} onChange={(e) => setResolutionText(e.target.value)} rows={4}
              placeholder="Qanday hal qilinganini yozing..."
            ...
            <div className="flex gap-2">
              ...
              <button onClick={submitResolve} disabled={!resolutionText.trim() || updatingId === resolving.id}
                ...
                <CheckCircle2 className="w-3.5 h-3.5" /> Hal qilindi deb belgilash
              </button>
            </div>
          </div>
        </div>
