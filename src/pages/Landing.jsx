...
const categoryTiles = [
  { name: "Frontend", icon: "💻", match: "frontend" },
  { name: "Backend", icon: "⚙️", match: "backend" },
  { name: "Mobile", icon: "📱", match: "mobile" },
  { name: "UI/UX", icon: "🎨", match: "ui/ux|dizayn|designer" },
  { name: "DevOps", icon: "🔧", match: "devops" },
  { name: "Ingliz tili", icon: "🇬🇧", match: "ingliz|english|ielts" },
  { name: "Matematika", icon: "📐", match: "matematika|math" },
  { name: "Fizika", icon: "🔬", match: "fizika|physics" },
];
...
export default function Landing() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [specData, statsData] = await Promise.all([
          api("/specialists"),
          api("/stats/overview"),
        ]);
  ...
  const filtered = withMatch.filter((s) => {
    ...
    if (activeFilter === "O'qituvchiler") return s.category === "Ta'lim" || (s.fields || []).includes("Ta'lim");
    if (activeFilter === "Senior") return s.experience_level === "Senior";
    if (activeFilter === "Online") return !!s.online;
    const q = activeFilter.toLowerCase();
    return s.skills.some((t) => t.toLowerCase().includes(q)) || s.category.toLowerCase().includes(q);
  });

  const categoryCounts = categoryTiles.map((cat) => ({
    ...cat,
    count: specialists.filter((s) => new RegExp(cat.match, "i").test(`${s.category} ${(s.skills || []).join(" ")}`)).length,
  }));

  const SpecialistCard = ({ s, showMatch }) => (
      ...
      <div className="bg-gradient-to-br from-ink via-ink/90 to-ink/70 text-white relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12 max-w-2xl mx-auto">
            {[
              { icon: Users, label: "Mutaxassislar", value: stats.specialists ?? "—" },
              { icon: Briefcase, label: "Vakansiyalar", value: stats.vacancies ?? "—" },
              { icon: Award, label: "Joylash", value: stats.placements ?? "—" },
              { icon: Globe, label: "Kompaniyalar", value: stats.companies ?? "—" },
            ].map((stat) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">
          ...
          <Link to="/vacancies" className="bg-gradient-to-br from-blue-600 to-blue-500 rounded-xl p-5 text-white relative overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="relative">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-medium bg-white/15 px-3 py-1.5 rounded-lg">
                {stats.vacancies ?? 0} ta vakansiya
              </span>
            </div>
          </Link>
        </div>

        {loading && <div className="text-center py-16 text-ink-3 text-sm">Yuklanmoqda...</div>}

        {!loading && (
          <>
            {/* Online Specialists */}
            {withMatch.some((s) => s.online) && (
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {withMatch.filter((s) => s.online).map((s) => (
                    <SpecialistCard key={s.id} s={s} showMatch={false} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {filtered.map((s) => (
                  <SpecialistCard key={s.id} s={s} showMatch />
                ))}
              </div>
            </div>
