  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
      <div className="flex items-center justify-between mb-5 md:mb-8">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-ink tracking-tight">{t("pages.orders.title")}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 mb-6 md:mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-xl border border-border p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center ${s.color}`}>
                <s.icon className="w-4 h-4 sm:w-[18px] sm:h-[18px]" />
              </div>
            </div>
            <div className="text-xl sm:text-2xl font-bold text-ink">{s.value || 0}</div>
            <div className="text-[10px] sm:text-xs text-ink-3 mt-1">{s.label}</div>
          </div>
        ))}
      </div>

      <div className="flex items-center gap-1.5 sm:gap-2 mb-5 md:mb-6 overflow-x-auto pb-2">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-xl border border-border p-3 sm:p-5 hover:border-ink/10 transition-colors">
