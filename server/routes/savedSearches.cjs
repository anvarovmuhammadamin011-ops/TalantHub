...
router.post("/", authMiddleware, async (req, res) => {
  try {
    ...
    const search = await db.prepare("SELECT *, 0 as match_count FROM saved_searches WHERE id = ?").get(result.lastInsertRowid);
    res.json({ search });
  } catch (err) {
...
router.delete("/:id", authMiddleware, async (req, res) => {
  try {
    const search = await db.prepare("SELECT * FROM saved_searches WHERE id = ?").get(req.params.id);
    ...
    await db.prepare("DELETE FROM saved_search_matches WHERE saved_search_id = ?").run(req.params.id);
    await db.prepare("DELETE FROM saved_searches WHERE id = ?").run(req.params.id);
