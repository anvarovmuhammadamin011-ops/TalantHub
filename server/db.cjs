// PostgreSQL-backed db module (previously better-sqlite3). Every route file does
// `const db = require("../db.cjs")` and then `db.prepare(sql).get/all/run(...)` — kept that
// exact shape so call sites only needed `async`/`await` added, not rewritten SQL.
// The one real difference from the old synchronous driver: every method is async, and
// `db.transaction()` runs a pooled client through BEGIN/COMMIT/ROLLBACK for the handful of
// call sites that need multi-statement atomicity.
